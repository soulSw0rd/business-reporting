...
def main():
    ...
    page = st.sidebar.selectbox(
        "Choisir une page",
        ["🏠 Vue d'ensemble", "👑 Top Traders", "📊 Analyse Crypto", "🔥 Données Live", "📈 Performance Portfolio", "⚙️ API Status"]
    )
    
    # Vérification des données disponibles
    scraped_data = get_scraped_data()
    api_data = get_api_data()
    crypto_data, traders_data = generate_crypto_sample_data()
    
    # Affichage selon la page sélectionnée
    if page == "🏠 Vue d'ensemble":
        show_overview(scraped_data, api_data, crypto_data, traders_data)
    elif page == "👑 Top Traders":
        show_top_traders(scraped_data, api_data, traders_data)
    elif page == "📊 Analyse Crypto":
        show_crypto_analysis(crypto_data)
    elif page == "🔥 Données Live":
        show_live_data()
    elif page == "📈 Performance Portfolio":
        show_portfolio_performance(crypto_data)
    elif page == "⚙️ API Status":
        show_api_status(api_data, scraped_data)

def show_overview(scraped_data, api_data, crypto_data, traders_data):
    ...
    with col1:
        api_status = "🟢 En ligne" if api_data else "🔴 Hors ligne"
        st.metric("API Status", api_status)
    
    with col2:
        scraped_files = len(scraped_data) if scraped_data else 0
        st.metric("Fichiers Data", scraped_files)
    
    with col3:
        active_traders = len(traders_data)
        st.metric("Top Traders", active_traders)
    
    with col4:
        crypto_pairs = len(crypto_data['Symbol'].unique())
        st.metric("Paires Crypto", crypto_pairs)
    ...
    with col1:
        st.subheader("📊 Performance des principales crypto")
        latest_prices = crypto_data.groupby('Symbol').last().reset_index()
        latest_prices = latest_prices.sort_values('Market_Cap', ascending=False).head(8)
    ...
    with col2:
        st.subheader("🎯 Distribution des Top Traders par ROI")
        fig = px.histogram(
            traders_data,
            x='roi_percentage',
            nbins=15,
            title="Distribution ROI des Top Traders",
            color_discrete_sequence=['#FFD700']
        )
        fig.update_layout(height=400)
        st.plotly_chart(fig, use_container_width=True)
    
    # Section des données en temps réel si disponibles
    if api_data:
        st.subheader("🔥 Données live de l'API")
        st.json(api_data)
    elif scraped_data:
        st.subheader("📁 Dernières données scrapées")
        for filename, data in scraped_data.items():
            with st.expander(f"📄 {filename}"):
                if isinstance(data, list) and data:
                    st.write(f"Nombre d'entrées: {len(data)}")
                    st.json(data[:3])  # Affiche les 3 premiers éléments
                else:
                    st.json(data)

def show_top_traders(scraped_data, api_data, traders_data):
    """Affiche l'analyse des top traders"""
    st.header("👑 Analyse des Top Traders")
    
    # Données des traders (priorité: scraped data, puis API, puis demo)
    traders_df = None
    ...
    if traders_df is None and api_data and 'traders' in api_data:
        traders_df = pd.DataFrame(api_data['traders'])
        st.success("📡 Données récupérées via API")
    
    if traders_df is None:
        traders_df = traders_data
        st.warning("⚠️ Utilisation des données de démonstration")
